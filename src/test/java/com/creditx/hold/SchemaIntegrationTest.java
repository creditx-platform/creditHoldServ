...
class SchemaIntegrationTest {
  ...
  void testFlywayAppliedSchema() {
    ...
    Integer processedEventsTableCount = jdbcTemplate.queryForObject(
        "SELECT COUNT(*) FROM user_tables WHERE table_name = 'CHS_PROCESSED_EVENTS'",
        Integer.class);
    assertThat(processedEventsTableCount).isEqualTo(1);

    // Test that CHS_HOLDS table exists
    Integer holdsTableCount = jdbcTemplate.queryForObject(
        "SELECT COUNT(*) FROM user_tables WHERE table_name = 'CHS_HOLDS'", Integer.class);
    ...
    Integer outboxEventsTableCount = jdbcTemplate.queryForObject(
        "SELECT COUNT(*) FROM user_tables WHERE table_name = 'CHS_OUTBOX_EVENTS'", Integer.class);
    ...
    Integer processedCount = jdbcTemplate.queryForObject(
        "SELECT COUNT(*) FROM CHS_PROCESSED_EVENTS WHERE EVENT_ID = ?", Integer.class,
        "test-event-123");
    ...
    Integer holdsCount = jdbcTemplate.queryForObject(
        "SELECT COUNT(*) FROM CHS_HOLDS WHERE TRANSACTION_ID = ?", Integer.class, 12345L);
