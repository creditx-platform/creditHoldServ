...
class HoldServiceImplTest {
    ...
    void shouldReturnExistingHoldForIdempotency() {
        ...
        verify(outboxEventService, times(0)).saveEvent(anyString(), any(), anyString()); // Should not create event
    ...
    void shouldRejectHighAmountTransactionForFraud() {
        // given
        CreateHoldRequest request = createHoldRequest(123L, new BigDecimal("15000.00")); // Exceeds fraud limit
    ...
    void shouldExpireHolds() {
        ...
        when(holdRepository.findExpiredHolds(eq(HoldStatus.AUTHORIZED), any(Instant.class))).thenReturn(expiredHolds);
    ...
    void shouldHandleNoExpiredHolds() {
        // given
        when(holdRepository.findExpiredHolds(eq(HoldStatus.AUTHORIZED), any(Instant.class))).thenReturn(Arrays.asList());
