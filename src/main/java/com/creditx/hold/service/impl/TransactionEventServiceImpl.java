...
public class TransactionEventServiceImpl implements TransactionEventService {
    ...
    public void processTransactionPosted(TransactionPostedEvent event) {
        ...
        try {
            ...
            // Mark event as processed
            processedEventService.markEventAsProcessed(eventId, payloadHash, "SUCCESS");
            log.info("Successfully processed transaction.posted event for transaction: {}", event.getTransactionId());
            ...
            log.error("Failed to process transaction.posted event for transaction: {}", event.getTransactionId(), e);
            // Mark event as processed with failed status
            processedEventService.markEventAsProcessed(eventId, "", "FAILED");
            throw e;
        }
    ...
    public void processTransactionFailed(TransactionFailedEvent event) {
        ...
        try {
            ...
            if (hold.getStatus() != HoldStatus.VOIDED && hold.getStatus() != HoldStatus.EXPIRED) {
                ...
                log.info("Hold {} is already in final state {}, no update needed", 
                        event.getHoldId(), hold.getStatus());
            }
            
            // Mark event as processed
            processedEventService.markEventAsProcessed(eventId, payloadHash, "SUCCESS");
            log.info("Successfully processed transaction.failed event for transaction: {}", event.getTransactionId());
            ...
            log.error("Failed to process transaction.failed event for transaction: {}", event.getTransactionId(), e);
            // Mark event as processed with failed status
            processedEventService.markEventAsProcessed(eventId, "", "FAILED");
            throw e;
        }
