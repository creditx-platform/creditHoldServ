package com.creditx.hold.config;
...
public class OpenApiConfig {
  ...
  @Bean
  public OpenAPI holdServiceOpenAPI() {
    OpenAPI api = new OpenAPI().info(new Info().title("CreditX Hold Service API")
            .description("Operations for placing, releasing and expiring credit holds / fraud locks")
            .version("v1")
        ...
        .externalDocs(new ExternalDocumentation().description("CreditX Platform Docs")
            .url("https://docs.creditx.local"));

    api.addTagsItem(
        new Tag().name("public").description("Publicly supported API (stable contract)"));
    api.addTagsItem(new Tag().name("internal")
        .description("Internal backend API (subject to change / not for external consumers)"));
    return api;
  }
  ...
  public OpenApiCustomizer internalTagHidingCustomiser() {
    return openApi -> {
      ...
      // Iterate paths and strip operations tagged as internal
      var pathsToRemove = new java.util.ArrayList<String>();
      openApi.getPaths().forEach((path, pathItem) -> {
        ...
        for (PathItem.HttpMethod method : PathItem.HttpMethod.values()) {
          ...
          if (isInternal(op)) {
            ...
            allRemoved = false; // at least one non-internal operation remains
          }
        }
        if (allRemoved) {
