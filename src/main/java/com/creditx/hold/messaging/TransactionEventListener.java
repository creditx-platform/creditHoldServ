  ...
  public Consumer<Message<String>> transactionPosted() {
    return message -> {
      ...
      try {
        ...
        TransactionPostedEvent event = objectMapper.readValue(payload,
            TransactionPostedEvent.class);
        transactionSpanTagger.tagTransactionId(event.getTransactionId());

        // Validate that the event has a holdId
        if (event.getHoldId() == null) {
          log.warn(
              "Skipping transaction.posted event without holdId for transaction: {} - payload: {}",
              event.getTransactionId(), payload);
          return;
        }
  ...
  public Consumer<Message<String>> transactionFailed() {
    return message -> {
      ...
      try {
        ...
        TransactionFailedEvent event = objectMapper.readValue(payload,
        ...
        if (event.getHoldId() == null) {
          log.warn(
              "Skipping transaction.failed event without holdId for transaction: {} - payload: {}",
              event.getTransactionId(), payload);
          return;
        }
